...
function getRandomInitialBallPosition(planets: Sphere[], ballRadius: number) {
	...
	const randomRadius = new Vector3(_.random(-0.5, 0.5), _.random(-0.5, 0.5), _.random(-0.5, 0.5))
		.normalize()
		.multiplyScalar(startingPlanet.radius)
...
export function generateRandomLevel(): Level {
	...
	const targetPlanet = _.sample(planets)!;
	const flagOffset = new Vector3(
		Math.random() - 0.5,
		Math.random() - 0.5,
		Math.random() - 0.5
	).normalize().multiplyScalar(targetPlanet.radius + 0.1);

	const level = {
		planets,
		initialBallPosition: getRandomInitialBallPosition(planets, settings.ball.radius),
		flagPosition: targetPlanet.position.clone().add(flagOffset),
	};
